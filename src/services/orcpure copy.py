...
class OrderFlowOrchestrator:
    ...
    def _update_local_metrics(self, packet):
        ...
        # Inject computed values into packet
    ...
    def _process_depth_update(self, packet):
        ...
        # 2. Full-Stack Imbalance (local computation — injected into packet for downstream use)
        ...
        # Inject the locally computed full-stack imbalance so all detectors use it
        packet['fs_imbalance'] = fs_imbalance
    ...
    def _check_pending_confirmations(self, symbol, ltp, packet, is_l3):
        ...
        On confirmation: calls _evaluate_trigger with the original trigger packet
        (not the current one) so entry price reflects when the surge fired, not
