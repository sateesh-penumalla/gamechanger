...
class MarketDepthService:
    ...
    def _setup_dhan_feed(self):
        ...
        # Always include major indices for market sentiment (VQS)
    ...
    def _run_dhan(self):
        ...
        for sym in sorted_symbols:
            ...
            # Future Meta (Separated for OI tracking, not merged into Cash Sentiment)
            fut_id = self.dhan_data.get_active_futures_id(sym)
            if fut_id:
                ...
                if fno_list:
                    ...
                    if fut_meta:
                        comprehensive_meta[fut_id] = {
                            "symbol": f"{sym}_FUT", # Append suffix to avoid clashing with Cash
                            "is_derivative": True,
